...
const fakeCallback = (eltsToColor, formIdname, checkColoredProgress) => () => {
  ...
  if (!checkColoredProgress) {
    ...
    eltsToColor.map((el) =>{
      ...
      if (arrr[0] !== undefined) {
        arrr[0].parentNode.classList.add(`checkFailFormIn`);
        setTimeout(() => {
          if (arrr[0] !== undefined) {
            arrr[0].parentNode.classList.remove(`checkFailFormIn`);
          }
      ...
      return <React.Fragment>{
        fields.map((el, index) => {
          if (el.type === `number` || el.type === `string`) {
            return <React.Fragment key={`FrOfinp${index}:${el.type}:${el.name}`}>
              {el.name}
              ...
              type='text' size={el.range} value={el.value} name={el.name} /><br/>
            </React.Fragment>;
          } else if (el.type === `textarea`) {
            return <React.Fragment key={`FrInptextA${index}:${el.type}:${el.name}`}>
              {el.name}
    ...
    renderBySchema(callback, nowAnkid) {
      ...
      return <React.Fragment>
        { schemaMap[nowAnkid].questions.map((el, index) => {
          return <React.Fragment key={`q:${el.value}:${index}`}>{el.value}<br/>
            <div className={`radioDivQs`}>
              {el.aids.map((el2, in2) => {
                return <React.Fragment key={`bySchema${el.qid}:${in2}:${el2}of${index}`}>
                  ...
                  type='radio'
                  className={el.addClasses === undefined ? `` : (el.addClasses.map((el3) => {
                    return `${el3}`;
                  }))}
